...
from lib.data_prep import ix_to_label, label_to_ix, tokenized_sentence, LABEL_TO_IX, IX_TO_LABEL, get_vocab, val_dataset, LABELS, raw_y_train, raw_y_val, raw_y_test, create_dataset, train_dataset
...
def compare_pca_features(target_label, save=False):
    ...
    if save: plt.savefig(f"visualizations/{target_label}_glove_plot.png", dpi=400, bbox_inches = "tight", pad_inches = 0.2, facecolor='w')
    ...
    plt.title("Number Count, Year Count")
    if save: plt.savefig(f"visualizations/{target_label}_glove_plot_added_features.png", dpi=400, bbox_inches = "tight", pad_inches = 0.2, facecolor='w')
    return pca_vals
...
sns.scatterplot(x='pc1', y='pc2', hue='Model Prediction', data=df, style='Correct?', markers={True:'o', False:'X'}, s=100)
ax.set_xticks([])
ax.set_yticks([])
fig.suptitle(f'"{target_label.title()}" Predictions with Added Features', fontsize=20)
plt.title("0.80 Accuracy, 0.64 F1-Score")
plt.savefig(f"visualizations/{target_label}_predictions.png", dpi=400, bbox_inches = "tight", pad_inches = 0.2, facecolor='w')
...
feature_counts = {}
for label in LABELS:
    feature_counts[LABEL_TO_IX[label]] = {0: {"year_count": [], "num_count": []}, 1: {"year_count": [], "num_count":[]}}
    for batch, targets, lengths, raw_data in create_dataset(
        val_dataset, id2tok, tok2id, label, batch_size=BATCH_SIZE
    ):
        num_count = len(re.findall(r"\b\d{1,3}(?:,\d{3})*(?:\.\d+)?(?!\d)", raw_data[0]))
        year_count = len(re.findall(r"\d{4}", raw_data[0]))
        feature_counts[LABEL_TO_IX[label]][targets.item()]['year_count'].append(year_count)
        feature_counts[LABEL_TO_IX[label]][targets.item()]['num_count'].append(num_count)
...
for label_ix, feat_data in feature_counts.items():
    ...
    mean_plus_minus[label_ix]['sum'] = (mean_pos_year - mean_neg_year) + (mean_pos_number - mean_neg_number)
...
for ix, (x, y) in enumerate(zip(X,Y)):
    fig.add_trace(go.Bar(x=[x], y=[IX_TO_LABEL[ix]], orientation='h', name=IX_TO_LABEL[ix], hovertemplate="%{y}: %{x}"))
...
plt.savefig("visualizations/label_distribution", dpi=400, bbox_inches = "tight", pad_inches = 0.2)
...
mutliclass_instances_arr = np.array([mutliclass_instances[ix] for ix in range(len(mutliclass_instances))])
df_cm = pd.DataFrame(
    class_corr_arr,
...
"""
ix_to_counts = {label_to_ix[label]: v for label, v in dict(Counter(flattened_y)).items()}
total_counts = np.array([ix_to_counts[ix] for ix in range(len(ix_to_counts))])
ordered_multilabel = dict(zip(LABELS, (mutliclass_instances_arr / total_counts).tolist())).items()
