...
class LSTMClassifier(nn.Module):
    def __init__(
        self, vocab_size, embedding_dim, hidden_dim, bidirectional, embed_weights=None
    ):
        ...
        if torch.is_tensor(embed_weights):
            ...
            self.embedding = nn.Embedding(
                vocab_size, embedding_dim, _weight=embed_weights
            )
    ...
    def init_hidden(self, batch_size):
        return (
            autograd.Variable(
                torch.randn(2 if self.bidirectional else 1, batch_size, self.hidden_dim)
            ),
            autograd.Variable(
                torch.randn(2 if self.bidirectional else 1, batch_size, self.hidden_dim)
            ),
    ...
    def forward(self, batch, lengths):
        ...
        embeds = self.embedding(
            batch
        )  # embeds = (largest_seq, batch_size, embedding_dim)
        ...
        """
        packed_output, (ht, ct) = self.lstm(
            packed_input, self.hidden
        )  # packed_output = (sum(lengths), hidden_dim)
        lstm_out, _ = pad_packed_sequence(
            packed_output
        )  # lstm_out = (sum(lengths), batch_size, hidden_dim)
